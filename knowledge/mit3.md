...
visit MIT OpenCourseWare at ocw.mit.edu.  PROFESSOR: So let's start with
...
of how a risk-neutral framework and risk-neutral pricing works. So we are, here,
...
is a liquid instrument which is traded on exchanges. And derivative may be
...
price is also a linear function but slightly shifted. And we'll see how come
...
terms, the pay-out is maximum of S minus K and 0. Right? And that's what happens at
...
risk preferences of market participants or counter-parties. It actually only depends on
...
$1 and basically nothing because interest rates are 0. So in time dt, we will assume
...
grows exponentially with interest rate r. So we substitute everything
...
that what we learned is that a is equal to df by dS. Then we collect all
...
derivative as any pay-off should satisfy this equation. The other observation
...
observe them graphically. So basically for call, as
...
there is some kind of expected volume going on. Right? One probability times another. But these are the formulas. And that's how I drew
...
tree methods or Monte Carlo simulations. And that's what was
...
means that our put, at any time t, is stock minus K. Right? So if we know all of the
